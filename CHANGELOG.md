...
- Suite e2e LIVE arreglada y refactorizada (WEDINGO_E2E_LIVE=1, 18 tests verdes): RsvpSection expone un puente `window.__updateRsvpField` SOLO en modo `__e2e` para que los tests escriban campos del formulario por el contexto React (la UI estilizada rv2-* es frágil para Playwright: inputs controlados + checkbox de privacidad); helper tipado `enableRsvpBridge`/`setRsvpField` en e2e/test-invite.ts; sobre abierto con clic nativo en `.envelope__panel--front`; RSVP con acompañantes sin desbordar verifica `.rv2-card`. Corregidos nombres de invitado a 3 palabras válidas. Test `filterByActivity` 'semana' de superadmin con fechas relativas a `Date.now()` (antes hardcodeadas de agosto 2026, fallaban según la fecha del sistema).
- Barra admin en la página PÚBLICA: `.admin-bar__link` (Invitación/Panel) vivía en admin.css (solo cargado en rutas de edición) → botones sin estilo hasta pulsarlos (también en producción). Movidos a public-shell.css (regresión del split de CSS v2.186).
...
- Auditoría periódica (AGENTS.md) de la v2.192.0 completada: seguridad (0 CVEs, CSP/headers, rules, sin sinks XSS), accesibilidad (193 tests axe), legalidad (GDPR/consentimiento/borrado), funcionalidad (2515 unit + 18 e2e) y rendimiento (JS inicial 107KB gz). Detalle en `AUDITORIA.md`.
## 2.191.0 — 2026-08-30
...
## 2.173.0 — 2026-08-28
- Modularización: el procesado de respuestas RSVP (descifrado de dietas y asociación de acompañantes) se extrae del hook a un helper puro, reduciendo useRsvp a la orquestación.
...
- Eliminados el selector de tarjetas anterior y su CSS huérfano.
...
- El panel de asistencias guarda los acompañantes sin el flag isChild (los datos ya existentes con isChild siguen mostrando su columna e historial).
## 2.145.0 — 2026-08-27
