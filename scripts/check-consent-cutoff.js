 ...
 * debe cumplir:
 *
 *   today - 14 years < CUTOFF <= today - 13 years
 *
 *   - Si CUTOFF <= today-14y  → un invitado que acaba de cumplir 14 (nacido hoy-14y)
 *     tendría birthDate >= CUTOFF y se le pediría consentimiento indebidamente.
 *   - Si CUTOFF >  today-13y  → algún invitado de 13 años quedaría fuera del filtro.
 *
 * Se recomienda fijar CUTOFF = (hoy - 14 años + 1 día) al actualizar.
 */
...
// Ventana válida: (hoy - 14 años, hoy - 13 años]
const lowerExclusive = subYears(today, 14); // today - 14y  (exclusive)
const upperInclusive = subYears(today, 13); // today - 13y  (inclusive)
...
if (cutoffDate.getTime() > upperInclusive) {
  console.error(
    `❌ La fecha de corte de consentimiento parental "${cutoff}" es demasiado reciente.\n` +
      `   CUTOFF debe ser <= ${new Date(upperInclusive).toISOString().slice(0, 10)} (hoy - 13 años) ` +
      `para no dejar fuera a invitados de 13 años.`,
  );
  process.exit(1);
}
