export const CHANGELOG = [
  ...
  },
  {
    ...
    date: "2026-07-28",
    changes: [
      "SEGURIDAD: gallery/audio delete requiere auth en Firestore rules, CSP style-src unsafe-inline, font-src data:",
      "SEGURIDAD: errorDetail i18n en backup/restore, notAttendingCount en rules, session renewal fix",
      "SEGURIDAD: PasswordCredential typings, vitest.setup.ts",
      "CALIDAD: cobertura 34.96%→~87%, 291→1598 tests, 141 test files",
      ...
      "UI: Contenedores invitación 90% base, 80% en móvil para mejor scroll",
      "UI: Botón accesibilidad visible en /admin, changelog modal 40%x80%",
      "UX: Export/restore rediseñado — export usa datos locales (sin re-fetch), restore multi-invitación",
      "UX: Secciones alojamiento/transporte movidas a 'Sobre los invitados' en setup",
      ...
      "BUGFIX: ConfigContext test OOM dividido en 3 archivos",
      "BUGFIX: image-store test flaky, restore error mensaje, i18n clave errorDetail",
  ...
  },
  {
    ...
    date: "2026-07-22",
    changes: [
      "AUDITORÍA COMPLETA: 50+ puntos analizados, exportados a AUDITORIA50ULTIMA",
      "UPDATE: APP_VERSION 2.16.8 → 2.25.0",
      "README.md actualizado con funcionalidad completa",
      "CHANGELOG actualizado con todas las versiones",
      "REFACTOR: Gallery CSS reescrito desde cero con variables de tema (--invite-core-color)",
      "FIX: Gallery-main-image-wrap aspect-ratio 16/10 (imágenes no se mostraban)",
      "FIX: Gallery responsive (≤768px) con tamaños adaptados",
      "FIX: user-select none en toda la página (excepto inputs/textarea)",
      "FIX: Audio sin límite de duración (canción completa, no truncada a 60s)",
      "FIX: Audio no usaba caché de sessionStorage (siempre consulta Firestore)",
      "FIX: Admin bar muestra username en vez de inviteToken",
      "FIX: Auto-save debounce 3s→1.5s",
      "FEAT: wind-sway animation en decoration (brisa ligera, 7s orgánico)",
      "FEAT: Login valida username contra adminUsername",
      "FEAT: Detección de fotos duplicadas en galería (nombre+tamaño)",
      "REFACTOR: gallery+audio como subcolecciones de invitations/{token}",
      "REFACTOR: setupTokens eliminado — todo en invitations._activeSetupToken",
      "REFACTOR: backgroundImage field eliminado (temas son el fondo)",
      "FIX: backgroundImage 100K gate eliminado, auto-save merge, image cap 800→400KB",
      "FIX: Audio comprimido sonaba lento (resampling manual→OfflineAudioContext)",
      "FIX: couple photo loading infinito corregido",
      "FIX: storyText limit 2000→500 caracteres",
      "FIX: story-card overflow-y scroll",
      "FIX: monthNames usa MONTH_VALUE_TO_NUMBER",
      "FIX: GalleryManager + useRsvp race condition",
      "FIX: i18n completo (themeNames, monthNames, ~30 keys faltantes)",
      "FIX: ~80 locale files con placeholders en español→inglés",
      "FIX: access.currentTokenPlaceholder huérfano eliminado de 100 archivos",
      "FIX: SupportTab 2 columnas + uploads unificados estilo galería",
      "FIX: ComplianceTab reestilizado",
      "FIX: DashboardTab con stats generales (no por invitación)",
      "FIX: AuthContext dep inestable + AdminPage pathname",
      "FIX: PanelTab addToast sin importar (RUNTIME ERROR)",
      "FIX: InvitationsTab key path incorrecto (superadmin.exportAllBtn)",
      "RSVP: cambiado a modelo familiar (textos y placeholders)",
      "CSS: --invite-core-color definido en 21 temas",
      "CSS: Envelope colores hardcodeados→variables de tema",
      "CSS: setup-accent en invitación→invite-core-color",
      "CSS: Gallery caption con fondo semi-transparente y glow del tema",
    ],
  },
  {
    version: "2.16.3",
  ...
  },
  {
    ...
    date: "2026-07-22",
    changes: [
      "Fix: restore usa updateDoc en vez de setDoc (preserva session sin rules conflict)",
    ],
  },
  {
    version: "2.17.1",
  ...
  },
  {
    ...
    date: "2026-07-21",
    changes: [
      "Traducciones completadas: 82/82 idiomas traducidos nativamente vía Google Translate",
      "Keys deprecadas eliminadas de todos los idiomas",
    ],
  },
  {
    version: "2.15.6",
