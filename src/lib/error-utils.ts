...
export function getFirestoreErrorMessage(error: unknown, t?: (key: string) => string): string {
  const code =
    error && typeof error === "object" && "code" in error ? String((error as Record<string, unknown>).code) : "";
...
export function logError(error: unknown, context?: string) {
  ...
  if (SENTRY_DSN && hasAnalyticsConsent()) {
    ...
    // .catch() final: si captureException lanza (p. ej. red caída o un mock
    // de test), el rechazo no debe convertirse en un unhandled rejection.
