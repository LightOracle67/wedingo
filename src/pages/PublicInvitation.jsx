    ...
    <div className={`app-scene ${isStoryTransitioning ? "app-scene--transitioning" : ""}`}>
      ...
      <a
        href={`https://wa.me/?text=${encodeURIComponent(
          `${config.firstName} & ${config.secondName} – ${formattedDate ? `Te esperamos el ${formattedDate}` : "Invitación de boda"}. ${window.location.origin}`,
        )}`}
        target="_blank"
        rel="noreferrer"
        className="fixed right-4 bottom-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-transform hover:scale-110 active:scale-95"
