...
function Section({ eyebrow, title, children, accent }: any) {
  return (
...
export default function PrintPage() {
  ...
  const sectionOrder = useMemo(() => {
    return parseSectionOrder(config.sectionOrder).filter((s: any) => s !== "rsvp" && !hiddenSet.has(s) && s !== "gallery");
        ...
        <Section eyebrow={t("print.aboutGuests")} title={t("print.scheduleTitle")} accent={accent} bg={bg}>
          ...
          {config.weddingDressCode ? (
            <div style={{ marginTop: 6 }}>
              {config.weddingDressCode.split(",").map((d: any, i: any) => <span key={i} className="print-dress">{escHtml(d.trim())}</span>)}
            </div>
