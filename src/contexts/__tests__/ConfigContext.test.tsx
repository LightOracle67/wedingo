...
vi.mock("../useAppUI", () => ({
  useAppUI: () => ({ setSaveMessage: vi.fn(), setSaveError: vi.fn() }),
}));
...
vi.mock("../../hooks/useAutoSave", () => ({
  useAutoSave: () => ({ autoSaveTimerRef: { current: null } }),
}));
...
vi.mock("../../lib/normalize-config", () => ({
  normalizeConfig: (v: unknown) => v,
}));

vi.mock("../../lib/date-utils", () => ({
  validateWeddingDate: () => null,
}));

vi.mock("../../lib/invite-config-codec", () => ({
  decodeInviteConfig: vi.fn(() => ({})),
}));
...
vi.mock("../../lib/image-store", () => ({
  loadDecryptedField: vi.fn(() => Promise.resolve("")),
...
vi.mock("../../lib/error-utils", () => ({
  getFirestoreErrorMessage: vi.fn(() => "error"),
}));
