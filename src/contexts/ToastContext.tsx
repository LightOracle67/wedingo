 ...
 */
function ToastProviderInner({ children, containerId = "toast-root", t }: any) {
   ...
   */
  const scheduleDismiss = useCallback((id: any, delay: any) => {
   ...
   */
  const addToast = useCallback((type: any, message: any, duration = 5000) => {
    const id = ++toastId;
   ...
   */
  const startUploadToast = useCallback((message: any) => {
    const id = ++toastId;
    ...
    const update = (percent: any) => {
      setToasts((prev) =>
        prev.map((t: any) => (t.id === id ? { ...t, progress: Math.min(100, Math.max(0, Math.round(percent))) } : t))
      );
    };
    ...
    const complete = (msg: any) => {
      setToasts((prev) =>
        prev.map((t: any) => (t.id === id ? { ...t, type: "success", message: msg, progress: 100 } : t))
      );
    ...
    const error = (msg: any) => {
      setToasts((prev) =>
        prev.map((t: any) => (t.id === id ? { ...t, type: "error", message: msg, progress: t.progress } : t))
      );
