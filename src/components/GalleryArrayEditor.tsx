...
const GalleryArrayEditor = memo(function GalleryArrayEditor({ inviteToken }: GalleryArrayEditorProps) {
  ...
  const handleUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>, slotIndex: number) => {
      ...
      const duplicate = slots.some(
        (s: SlotState | null) => s && s.originalName === file.name && s.originalSize === file.size,
      );
      if (duplicate) {
        addToast("warning", t("setup.duplicateFileWarning"));
        if (input) input.value = "";
        return;
      }
  ...
  const handleDescriptionBlur = useCallback(
    async (slotIndex: number, currentValue: string) => {
      if (!inviteToken) return;
      ...
      const safe = String(currentValue ?? "")
        .slice(0, 200)
        .trim();
      try {
  ...
  const handleMove = useCallback(
    async (slotIndex: number, dir: -1 | 1) => {
      if (!inviteToken) return;
  ...
  return (
    <div>
      <p className="setup-help" style={{ marginBottom: "0.5rem" }}>
        {t("setup.galleryHint")}
      </p>
        ...
        {Array.from({ length: SLOT_COUNT }, (_, i) => {
          ...
          return (
            <div key={i} style={galleryItemStyle}>
              ...
              >
                <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--setup-muted)" }}>#{i + 1}</span>
                {item ? (
