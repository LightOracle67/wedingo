...
const GalleryManager = memo(function GalleryManager({ images, onChange, inviteToken, t }: any) {
  const handleDelete = useCallback(async (index: any, item: any) => {
  ...
  const handleDescriptionChange = useCallback(async (index: any, val: any) => {
    const arr = JSON.parse(images || "[]");
    arr[index] = typeof arr[index] === "string" ? { url: arr[index], description: val } : { ...arr[index], description: val };
    onChange(JSON.stringify(arr));
  }, [images, onChange]);

  const handleBlur = useCallback(async (docId: any, description: any) => {
    if (!docId) return;
    try {
